...
export default function ServicesGrid() {
  return (
    <section id="servicios" className="bg-paper-2 py-24 md:py-28">
          ...
          {services.map((svc, i) => {
            ...
            return (
              <ScrollReveal
                key={svc.n}
                delay={i * 0.06}
                className={`col-span-1 ${spans[i]} group relative rounded-lg border border-ink/10 bg-paper-soft p-8 transition-all duration-300 hover:border-lavender/50 hover:shadow-lg`}
              >
                <span className="absolute top-6 right-7 font-display text-lg text-muted-dark/50">{svc.n}</span>
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-lg border border-ink/10 bg-paper text-lavender-deep transition-colors duration-300 group-hover:bg-lavender group-hover:text-ink">
